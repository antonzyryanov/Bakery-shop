...
-- 3NF schema:
-- roles        = role lookup (no repeating role descriptors on users)
-- users        = identity + role_code FK
-- dishes       = dish attributes (name, image, macros, description)
-- food_entries = consumption facts only (user + dish + eaten_at)
